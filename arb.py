...
for i in range(len(arb_binance_bybit)):
    print(f"Binance-Bybit: {bin_d[:,0][i]} - {by_d[:,0][i]} - {bin_d[:,2][i]} - {by_d[:,1][i]}: {arb_binance_bybit[i]} {minimum_amount_binance_bybit}")
    print(f"Bybit-Binance: {bin_d[:,0][i]} - {by_d[:,0][i]} - {by_d[:,2][i]} - {bin_d[:,1][i]}: {arb_bybit_binance[i]} {minimum_amount_binance_bybit}")
