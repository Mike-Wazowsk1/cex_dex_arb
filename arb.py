...
for i in range(len(arb_binance_bybit)):
    print(f"Bybit-Binance: {bin_d[:,0][i]} - {round(bin_d[:,2][i],6)} - {round(by_d[:,1][i],6)}: {arb_binance_bybit[i]} {minimum_amount_binance_bybit[i]}")
    print(f"Binance-Bybit: {bin_d[:,0][i]} - {round(by_d[:,2][i],6)} - {round(bin_d[:,1][i],6)}: {arb_bybit_binance[i]} {minimum_amount_binance_bybit[i]}")
